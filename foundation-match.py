...
def suggest(ret,n):
    ...
    for i in range(n):
        a = min(ret)
        b = ret.index(a)
        nm[col['Name'][b]] = [col['Shade'][b],col['Color'][b]]
        ret.remove(a)
    return nm
...
col = pd.read_csv("colors.csv")


#callfromform
faceb = cv2.imread(extractFace(face))
ip = pipeline(faceb)
ipw = pipeline(wrist)
ipwu = pipeline2(wrist)
